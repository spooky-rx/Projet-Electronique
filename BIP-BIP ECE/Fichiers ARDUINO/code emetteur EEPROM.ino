...
// ================= EEPROM (TX) =================
// Stocke seulement pseudoIndex
#define EE_TX_MAGIC_ADDR   0
#define EE_TX_MAGIC_VALUE  0xB2
#define EE_TX_VER_ADDR     1
#define EE_TX_VER_VALUE    0x01
#define EE_TX_PSEUDO_ADDR  2
#define EE_TX_CRC_ADDR     3

uint8_t crc8_tx(uint8_t pseudoIdx) {
  return (uint8_t)(pseudoIdx ^ 0x5A ^ EE_TX_MAGIC_VALUE ^ EE_TX_VER_VALUE);
}

void eepromTxLoad(uint8_t &pseudoIdx, uint8_t maxPseudos) {
  uint8_t m = EEPROM.read(EE_TX_MAGIC_ADDR);
  uint8_t v = EEPROM.read(EE_TX_VER_ADDR);
  uint8_t p = EEPROM.read(EE_TX_PSEUDO_ADDR);
  uint8_t c = EEPROM.read(EE_TX_CRC_ADDR);

  if (m != EE_TX_MAGIC_VALUE || v != EE_TX_VER_VALUE) {
    pseudoIdx = 0;
    return;
  }
  if (c != crc8_tx(p)) {
...
void eepromTxSave(uint8_t pseudoIdx) {
  EEPROM.update(EE_TX_MAGIC_ADDR, EE_TX_MAGIC_VALUE);
  EEPROM.update(EE_TX_VER_ADDR, EE_TX_VER_VALUE);
  EEPROM.update(EE_TX_PSEUDO_ADDR, pseudoIdx);
  EEPROM.update(EE_TX_CRC_ADDR, crc8_tx(pseudoIdx));
}
...
void ledBlinkGreen(int n) { for (int i=0;i<n;i++){ pwmWrite(LED_G,255); delay(120); ledOff(); delay(120);} }
void ledBlinkRed(int n)   { for (int i=0;i<n;i++){ pwmWrite(LED_R,255); delay(120); ledOff(); delay(120);} }
void buzzerBeep2() {
  for (int i=0;i<2;i++){ tone(BUZZER_PIN,2000); delay(120); noTone(BUZZER_PIN); delay(120); }
}

// ================= Pseudo menu =================
const char* pseudos[] = {"Mathieu", "Jules", "Tristan"};
const uint8_t PSEUDO_COUNT = sizeof(pseudos)/sizeof(pseudos[0]);
uint8_t pseudoIndex = 0;
const char* pseudo = "Mathieu";
...
// ================= UI state =================
enum UiState { STATE_MENU_PSEUDO, STATE_KEYBOARD };
UiState uiState = STATE_MENU_PSEUDO;
...
void setupRadio() {
  pinMode(SS, OUTPUT); // SS = D10 sur Nano (ici buzzer), doit rester OUTPUT => OK
  radio.begin();
...
bool sendMessageNRF24() {
  ...
  size_t dataLen = psLen + 1 + msgLen; // pseudo '\0' message
...
void drawKeyboard() {
  ...
  // preview 2 lignes
  const int charsPerLine = 21, lines = 2, maxChars = charsPerLine * lines;
  int start = 0;
  ...
  display.print("Page: ");
  display.print(specialPage ? "SPEC" : "AZ09");

  int y0 = 40, cellW = 12, cellH = 8;
  for (uint8_t r=0;r<ROWS;r++){
    for (uint8_t c=0;c<COLS;c++){
      int x = c * cellW;
      int y = y0 + r * cellH;
      if (r==selR && c==selC) display.drawRect(x, y, cellW, cellH, SSD1306_WHITE);

      display.setCursor(x + 2, y + 1);
      char k = specialPage ? kb2[r][c] : kb[r][c];
  ...
  display.print("Clic:add  SW3:send");
  display.display();
}
...
void setup() {
  ...
  // EEPROM: charger pseudoIndex
  eepromTxLoad(pseudoIndex, PSEUDO_COUNT);
...
void loop() {
  ...
  if (!lastSW3Pressed && sw3 && uiState == STATE_KEYBOARD) {
    ...
    if (ok) {
      ledBlinkGreen(3);
      oledStatus("Envoye OK", "SW3 pour renvoyer");
    } else {
      ledBlinkRed(3);
      oledStatus("Echec envoi", "Verif NRF24");
    }
    delay(400);
  }
  lastSW3Pressed = sw3;

  if (uiState == STATE_MENU_PSEUDO) {
    long idx = ticks % PSEUDO_COUNT;
    if (idx < 0) idx += PSEUDO_COUNT;
    pseudoIndex = (uint8_t)idx;

    if (encShortClick) {
      pseudo = pseudos[pseudoIndex];
      // EEPROM: sauvegarder le choix
      eepromTxSave(pseudoIndex);
  ...
  // STATE_KEYBOARD: déplacement
  long idx = ticks % (ROWS * COLS);
  if (idx < 0) idx += (ROWS * COLS);
  selR = (uint8_t)(idx / COLS);
  selC = (uint8_t)(idx % COLS);

  // clic encodeur = ajouter
  if (encShortClick) {
    char k = specialPage ? kb2[selR][selC] : kb[selR][selC];

    // toggle page : clic sur '?' (pratique à expliquer)
    if (k == '?') {
