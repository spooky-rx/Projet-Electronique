...
const uint8_t PIPE_RX[6] = "BIP1";   // doit matcher le PIPE_TX de l'émetteur
uint8_t RADIO_CH = 42;               // doit matcher l'émetteur
...
// ===== RX reassembly (simple, trames dans l'ordre) =====
...
void ledSetRGB(uint8_t r, uint8_t g, uint8_t b) {
  pwmWrite(LED_R, r); pwmWrite(LED_G, g); pwmWrite(LED_B, b);
}

void ledSetPriority(uint8_t prio) {
  if (prio == 0)      ledSetRGB(0,255,0);   // vert
  else if (prio == 1) ledSetRGB(0,0,255);   // bleu
  else                ledSetRGB(255,0,0);   // rouge
}
...
bool appendPayloadIfInOrder(const uint8_t* payload, uint8_t payloadLen, uint8_t seq) {
  if (seq != nextExpectedSeq) return false;
...
// ==================== UI menu ====================
bool menuOpen = false;
int menuIndex = 0; // 0..2 (LED mode / LED color / Bip)
