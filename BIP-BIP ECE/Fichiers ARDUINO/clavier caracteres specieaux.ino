...
const int PSEUDO_COUNT = sizeof(pseudos)/sizeof(pseudos[0]);
int pseudoIndex = 0;
...
// ================= Clavier grille =================
// On affiche une grille 4x10. Chaque "touche" est un caractère ASCII.
// Touches spéciales:
// ' ' => SP (espace)
// '<' => BS (backspace)
// On met des caractères spéciaux dans la grille (ponctuation + symboles).
...
// Une “page” de caractères spéciaux supplémentaire (accès via touche '#')
...
// La touche '#' (sur page spéciale) sert aussi à revenir à la page principale.
// Sur page normale, on utilise 'Z' ou autre ? -> on préfère basculer via un long clic ?
// Ici on bascule via clic long encodeur pour être simple.
...
void loop() {
  ...
  // STATE_KEYBOARD
  // map ticks -> index clavier (ROWS*COLS)
  long idx = ticks % (ROWS * COLS);
  if (idx < 0) idx += (ROWS * COLS);
  selR = (uint8_t)(idx / COLS);
  selC = (uint8_t)(idx % COLS);

  // clic encodeur = ajouter caractère
  if (encShortClick) {
    char k = specialPage ? kb2[selR][selC] : kb[selR][selC];

    // bascule page si on clique sur '?' par exemple (au choix)
    // Ici: clic sur '?' -> toggle page (pratique)
    if (k == '?') {
